...
export async function checkBackendHealth(): Promise<ApiStatus> {
  ...
  // Real CustomerIQ health path first, then fallbacks
 ...
 *
 * IMPORTANT: The real CustomerIQ backend does NOT expose /analyze or /predict
 * POST endpoints. Every route is a GET against an existing customer_id that
 * already lives in the SQLite database (Sql/customeriq.db). This function
 * looks up an existing customer's retention intelligence rather than
 * submitting arbitrary stats for scoring.
 */
export async function analyzeCustomer(input: CustomerInput): Promise<CustomerIntelligenceResult> {
  if (!input.customerId) {
    throw new ApiError('A customer ID is required to look up retention intelligence.', 'validation');
  }

  try {
    const response = await apiClient<any>(
      `/api/customers/${encodeURIComponent(input.customerId)}/retention-intelligence`
    );
...
/**
 * Fetch a single customer's core risk record (GET /api/customers/{customer_id})
 */
export async function fetchCustomer(customerId: string | number): Promise<any> {
  return apiClient<any>(`/api/customers/${encodeURIComponent(String(customerId))}`);
}

/**
 * Fetch a paginated / filtered customer list (GET /api/customers)
 */
export async function fetchCustomers(params: {
  ...
  return apiClient<any>(`/api/customers${qs ? `?${qs}` : ''}`);
}

/**
 * Fetch priority customers (GET /api/customers/priority)
 */
export async function fetchPriorityCustomers(limit: number = 20): Promise<any> {
  return apiClient<any>(`/api/customers/priority?limit=${encodeURIComponent(String(limit))}`);
}

/**
 * Fetch risk distribution (GET /api/analytics/risk-distribution)
 */
export async function fetchRiskDistribution(): Promise<any> {
  return apiClient<any>('/api/analytics/risk-distribution');
}

/**
 * Fetch value-vs-risk scatter data (GET /api/analytics/value-vs-risk)
 */
export async function fetchValueVsRisk(limit: number = 100): Promise<any> {
  return apiClient<any>(`/api/analytics/value-vs-risk?limit=${encodeURIComponent(String(limit))}`);
}

/**
 * Fetch aggregate metrics from the real /api/overview endpoint.
 */
export async function fetchAggregateMetrics(): Promise<AggregateMetrics | null> {
  const metricEndpoints = ['/api/overview'];
